import { useState } from 'react';
import { ArrowLeft, Copy, Eye, EyeOff, ExternalLink, Download } from 'lucide-react';
import { GlassCard } from '../../components/UIComponents';
import Footer from '../../components/Footer';
import { useTheme } from '../../contexts/ThemeContext';

const MT5CredentialsPage = ({ onBack }) => {
  const { isDark } = useTheme();
  const [showPassword, setShowPassword] = useState(false);

  const mt5Credentials = {
    server: 'RailTrader-Live',
    login: '2024001234',
    password: 'Tr@de2024!',
    investor: 'View2024!'
  };

  const copyToClipboard = (text) => {
    navigator.clipboard.writeText(text);
  };

  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 pb-20 md:pb-0">
      <div className="flex items-center gap-4">
        <button onClick={onBack} className={`p-2 rounded-xl ${isDark ? 'bg-white/10 text-white' : 'bg-gray-100 text-gray-900'}`}>
          <ArrowLeft size={20} />
        </button>
        <div>
          <h1 className={`text-2xl md:text-4xl font-bold ${isDark ? 'text-white' : 'text-gray-900'} tracking-tighter`}>MT5 Credentials</h1>
          <p className={`${isDark ? 'text-white/60' : 'text-gray-600'} text-sm`}>Your trading account details</p>
        </div>
      </div>

      <GlassCard className="p-6">
        <div className="flex items-center gap-3 mb-6">
          <ExternalLink className="text-blue-400" size={24} />
          <h2 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>Trading Account</h2>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className={`p-6 rounded-2xl ${isDark ? 'bg-white/5' : 'bg-gray-50'}`}>
            <div className="flex items-center justify-between mb-2">
              <p className={`text-sm font-medium ${isDark ? 'text-white/60' : 'text-gray-600'}`}>Server</p>
              <button onClick={() => copyToClipboard(mt5Credentials.server)} className="p-1 hover:bg-white/10 rounded">
                <Copy size={14} className={isDark ? 'text-white/40' : 'text-gray-400'} />
              </button>
            </div>
            <span className={`font-mono text-lg ${isDark ? 'text-white' : 'text-gray-900'}`}>{mt5Credentials.server}</span>
          </div>

          <div className={`p-6 rounded-2xl ${isDark ? 'bg-white/5' : 'bg-gray-50'}`}>
            <div className="flex items-center justify-between mb-2">
              <p className={`text-sm font-medium ${isDark ? 'text-white/60' : 'text-gray-600'}`}>Login</p>
              <button onClick={() => copyToClipboard(mt5Credentials.login)} className="p-1 hover:bg-white/10 rounded">
                <Copy size={14} className={isDark ? 'text-white/40' : 'text-gray-400'} />
              </button>
            </div>
            <span className={`font-mono text-lg ${isDark ? 'text-white' : 'text-gray-900'}`}>{mt5Credentials.login}</span>
          </div>

          <div className={`p-6 rounded-2xl ${isDark ? 'bg-white/5' : 'bg-gray-50'}`}>
            <div className="flex items-center justify-between mb-2">
              <p className={`text-sm font-medium ${isDark ? 'text-white/60' : 'text-gray-600'}`}>Password</p>
              <div className="flex items-center gap-2">
                <button onClick={() => setShowPassword(!showPassword)} className="p-1 hover:bg-white/10 rounded">
                  {showPassword ? <EyeOff size={14} /> : <Eye size={14} />}
                </button>
                <button onClick={() => copyToClipboard(mt5Credentials.password)} className="p-1 hover:bg-white/10 rounded">
                  <Copy size={14} className={isDark ? 'text-white/40' : 'text-gray-400'} />
                </button>
              </div>
            </div>
            <span className={`font-mono text-lg ${isDark ? 'text-white' : 'text-gray-900'}`}>
              {showPassword ? mt5Credentials.password : '••••••••••'}
            </span>
          </div>

          <div className={`p-6 rounded-2xl ${isDark ? 'bg-white/5' : 'bg-gray-50'}`}>
            <div className="flex items-center justify-between mb-2">
              <p className={`text-sm font-medium ${isDark ? 'text-white/60' : 'text-gray-600'}`}>Investor Password</p>
              <button onClick={() => copyToClipboard(mt5Credentials.investor)} className="p-1 hover:bg-white/10 rounded">
                <Copy size={14} className={isDark ? 'text-white/40' : 'text-gray-400'} />
              </button>
            </div>
            <span className={`font-mono text-lg ${isDark ? 'text-white' : 'text-gray-900'}`}>{mt5Credentials.investor}</span>
          </div>
        </div>

        <div className={`mt-6 p-4 rounded-2xl ${isDark ? 'bg-blue-500/10 border-blue-500/20' : 'bg-blue-50 border-blue-200'} border`}>
          <p className={`text-sm ${isDark ? 'text-blue-300' : 'text-blue-700'} mb-3`}>
            Use these credentials to login to your MT5 trading platform. Keep them secure and do not share with anyone.
          </p>
          <button className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-xl font-medium text-sm">
            <Download size={16} />
            Download MT5 Platform
          </button>
        </div>
      </GlassCard>

      <Footer />
    </div>
  );
};

export default MT5CredentialsPage;