...
const ChallengesPage = ({ onGetStarted, onNavigate, onAuthRequest }) => {
  ...
  const challenges = [
    { id: 1, name: "$10k Starter", amount: 10000, tier: 1, profitTarget: 10, maxDrawdown: 5, leverage: "1:100", price: 99, icon: Star },
    { id: 2, name: "$100k Pro", amount: 100000, tier: 2, profitTarget: 10, maxDrawdown: 5, leverage: "1:100", price: 499, icon: Zap, popular: true },
    { id: 3, name: "$250k Elite", amount: 250000, tier: 3, profitTarget: 10, maxDrawdown: 5, leverage: "1:100", price: 999, icon: Crown }
  ];
            ...
            {challenges.map((challenge, i) => (
              <GlassCard key={challenge.id} className={`p-6 md:p-8 flex flex-col border-2 ${challenge.popular ? 'border-blue-500/50 scale-105' : (isDark ? 'border-white/10' : 'border-gray-200')} hover:scale-105 transition-all duration-300`}>
                ...
                <div className={`space-y-3 mb-8 text-sm ${isDark ? 'text-white/70' : 'text-gray-600'}`}>
                  ...
                  <div className="flex justify-between font-bold"><span>Price</span><span>${challenge.price}</span></div>
                </div>
